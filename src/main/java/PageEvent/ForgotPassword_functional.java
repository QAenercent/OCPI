...
public class ForgotPassword_functional extends BaseTest {
	...
	public void verifyifloginpageisloaded() {
		 ...
		 Assert.assertTrue(ele.getWebElements("XPATH",ForgotPasswordObject.email).size()>0, "Page Not Loaded Properly");
	 }
	...
	public void validatealltypeof_toast() throws InterruptedException, IOException {
		...
		for (int i =1 ; i<=rows; i++) { 
			...
			ele.getWebElement("XPATH", ForgotPasswordObject.email).sendKeys(sheet.getRow(i).getCell(j++).toString());  
			ele.getWebElement("XPATH", ForgotPasswordObject.resetpassbutton).click();
			
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));
        	wait.until(ExpectedConditions.visibilityOf(ele.getWebElement("XPATH",ForgotPasswordObject.toastframe)));
		...
		if (ele.getWebElement("XPATH",ForgotPasswordObject.toastframe).isDisplayed()) {
		 ...
		 //******************* Validate Minimum & Maximum error Message  ****************************//
		  ...
		  if(ele.getWebElement("XPATH",ForgotPasswordObject.minimumlimit_error).isDisplayed()) {
			 ...
			 Assert.assertEquals(ele.getWebElement("XPATH",ForgotPasswordObject.minimumlimit_error).getText(),minandmax_error);
